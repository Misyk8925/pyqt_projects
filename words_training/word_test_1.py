...
class DlgMain(QDialog):
    ...
    def __init__(self):
        ...
        self.engl_lst = [word for word in self.english_words]
        self.grmn_lst = [word  for word in self.german_words]

        self.res_german = [str(word)[:-1] for word in self.grmn_lst[:-1]]
        self.res_german.append(self.grmn_lst[-1])
        self.res_english = [str(word)[:-1] for word in self.engl_lst[:-1]]
        self.res_english.append(self.engl_lst[-1])
        
        #print(self.res_english, self.res_german)
